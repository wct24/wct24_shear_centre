...
def main():
    ...
    f = part.faces
    print(f)
    part.PartitionFaceByAuto(face=f[23])
    part.seedPart(size=0.05, deviationFactor=0.001, minSizeFactor=0.01)
    part.generateMesh()
    ...
    odb = session.odbs['E:/temp/reid_section/analysis.odb']
    session.fieldReportOptions.setValues(sort=DESCENDING,
        reportFormat=COMMA_SEPARATED_VALUES)
    session.writeFieldReport(fileName='E:/temp/reid_section/analysis.csv', append=OFF,
