...
#import from the find sc using w
inside_points = pd.read_csv(r'E:\temp\SC\out.csv', header = 0,usecols = [1, 2, 3])
...
def shear_centre(point, df):
    ...
    index = df.index
    # find y coordinte
    condition = (abs(df['x']-x)<= 30*dx+0.000001) & (df['y'] == y)
    ...
    y_sc = y +rs
     # find x coordinte
    condition = (abs(df['y']-y)<= 30*dx+0.000001) & (df['x'] == x)
    b = index[condition].tolist()
    df_y = df.iloc[b]
