...
def get_sc_from_w(DF):
    ...
    plt.scatter(x_sc, y_sc, marker="x", color="r")

    theta_array= np.zeros(len(x_0))

    for i in range(len(x_0)):
        v1 = np.array([x_1[i]-x_0[i],y_1[i]-y_0[i]])
        v2 = np.array([x_sc-x_0[i],y_sc-y_0[i]])
        theta = np.linalg.norm(v1)/np.linalg.norm(v2)
        theta_array[i] = theta
    twist_orignal = np.mean(theta_array)


    a = [0.9, 0.92,0.95,0.97,1,1.1,1.3,1.4]
    spread_list = []
    for i in a:
        twist = twist_orignal*i
        ...
        for i in range(len(element_ids)):
            ...
            # ax.scatter(sc_list[:,0],sc_list[:,1], color='c', )
            a = np.array(sc_list)
            sc = np.mean(a, axis=0)
            # ax.scatter(sc[0],sc[1], color='c', )
            spread = np.linalg.norm(np.std(a, axis=0, dtype=np.float64))
            print("shear_centre : ",sc, " /  std : ", spread)
