...
# be careful to make sure that the csv matches to the index
u = pd.read_csv(r'E:\temp\c_section\analysis.csv', header = 0, usecols = [3, 12,13,14,16,17,18])
...
def plot_2d_space(DF,z):
    ...
    d = {'x': x_0, 'y': y_0, "w":warping_function}
    ...
    print(np.shape(np.linspace(x_min, x_max, number_of_points)))
    xi, yi = np.meshgrid(np.linspace(x_min, x_max, number_of_points), np.linspace(y_min, y_max, number_of_points))
    print(np.shape(xi))
    ...
    ax.set_zlabel('Z')
    plt.show()
    area = ((x_max-x_min)/number_of_points) * ((y_max-y_min)/number_of_points)
