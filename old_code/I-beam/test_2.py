...
def plot_2d_space(DF,z):
    ...
    alpha = 150#* alphashape.optimizealpha(points)
    ...
    print(np.shape(np.linspace(x_min, x_max, num)))
    xi, yi = np.meshgrid(np.linspace(x_min, x_max, num), np.linspace(y_min, y_max, num))
    print(np.shape(xi))
    ...
    ax.set_zlabel('Z')
    plt.show()
    area = ((x_max-x_min)/num) * ((y_max-y_min)/num)
    ...
    for i in range(len(xi)):
        for j in range(len(xi)):
            ...
            Boolean  = p1.within(poly)
            if Boolean == True:
                inside_points.append([xi[i][j],yi[i][j]])
                I_xx += area*(yi[i][j])**2
                I_yy += area*(xi[i][j])**2
                Q_y += area*(xi[i][j])
                Q_x += area*(yi[i][j])
    ...
    for i in range(len(xi)):
        for j in range(len(xi)):
            ...
            Boolean  = p1.within(poly)
            if Boolean == True:
                I_wx += area*Q_x*(zi_cubic_geom[i][j])
                I_wy += area*Q_y*(zi_cubic_geom[i][j])
            else:
                pass
